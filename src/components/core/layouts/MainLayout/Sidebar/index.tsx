...
export default function SideBar() {
    return (
        <div className="w-[88px] min-h-screen bg-white p-3">
            <div className='flex flex-col justify-between h-full w-full bg-white rounded-[14px] [box-shadow:0_1px_4px_rgba(0,0,0,0.2)] p-[8px]'>
...
export const SidebarItem = ({ icon, activeIcon, href }: SidebarItemProps) => {
    ...
    return (
        <button className={`w-full h-fit flex items-center justify-center p-3 cursor-pointer hover:bg-gray-100 rounded-xl transition-all`} onClick={() => href && router.push(href)}>
...
export const SideBarItemUnRoute = ({ icon, activeIcon, unRouteKey }: SidebarItemProps) => {
    ...
    return (
        <button
            className={`w-full h-fit flex items-center justify-center p-3 cursor-pointer hover:bg-gray-100 rounded-xl transition-all`}
...
export const SettingItem = () => {
    ...
    return <Popover placement="right" onOpenChange={(open) => setIsOpen(open)} className=''>
        <PopoverTrigger>
            <div className='w-full h-fit flex items-center justify-center p-3 cursor-pointer  hover:bg-gray-100 rounded-xl transition-all'>
                <GearIcon size={24} weight={isOpen ? 'fill' : 'light'} />
            </div>
        </PopoverTrigger>
        <PopoverContent>
            <div className="px-3 py-4">
                <div className="text-small font-bold mb-2">Settings</div>
                <div className="flex flex-col gap-2">
                    <button className="text-tiny hover:bg-gray-100 p-2 rounded-lg flex items-center gap-2 w-full">
                        Account Settings
                    </button>
                    <button className="text-tiny hover:bg-gray-100 p-2 rounded-lg flex items-center gap-2 w-full">
                        Privacy & Security
                    ...
                    <button className="text-tiny hover:bg-gray-100 p-2 rounded-lg flex items-center gap-2 w-full text-red-500">
                        Log Out
                    </button>
                </div>
            </div>
